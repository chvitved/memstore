...
object LoadData {
  ...
  val firstNames = loadNames("datafiles/firstnames.txt")
  val middleNames = loadNamesWithNulls("datafiles/sirnames.txt", 0.2)
  val sirnames = loadNames("datafiles/middlenames.txt")
  val roads = Source.fromFile("datafiles/out/roads.txt").getLines().toArray 
  val zipCodes: Array[ZipCode] = Source.fromFile("datafiles/zipcodes.txt").getLines().map{l => 
      if (l.contains("-") && l.split("  ").head.contains("-")) {
