...
class EntityTest {
  ...
  @Test()
  def testNoKeyValue() {
    val errorLikeString = "key value cannot be null"
    
    val p = CompactEntity.emptyDataPool
    val ed = EntityData(EntityConfig("test"))
    
    try {
    	val (ed2, p2) = ed + (new Date(), Map[String, Any]("a" -> "hello"), p)
    	fail()
    } catch {
      case e: Exception => assertTrue(e.getMessage.contains(errorLikeString))
    }
    
    try {
    	val (ed2, p2) = ed + (new Date(), Map[String, Any]("id" -> null, "a" -> "hello"), p)
    	fail()
    } catch {
      case e: Exception => assertTrue(e.getMessage.contains(errorLikeString))
    }
