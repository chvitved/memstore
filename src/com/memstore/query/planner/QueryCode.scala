...
object QueryCode {
  ...
  private def groupCode(group: List[ExpressionPlan], ed: EntityData, pool: CompactEntityDataPool, parameters: IndexedSeq[Any], date: Date, resultSoFar: Set[Entity], intersect: Boolean): Set[Entity] = {
    val sorted = group.sortWith((e1, e2) => e1.score < e2.score)
  ...
  private def leafExpCode(expPlan: LeafExpPlan, ed: EntityData, pool: CompactEntityDataPool, parameters: IndexedSeq[Any], date: Date, resultSoFar: Set[Entity], intersect: Boolean): Set[Entity] = {
    ...
    if (intersect) {
      if (limitForUsingIndex < resultSoFar.size || expPlan.scanType == FullScan) {
    	...
    	resultSoFar.intersect(usePrimaryIndex(expPlan.operator, ed, pool, parameter, date))
      ...
      val res: Set[Entity] = expPlan.scanType match {
        case PrimaryKeyScan => usePrimaryIndex(expPlan.operator, ed, pool, parameter, date)
        case IndexScan => useIndex(expPlan, ed, pool, parameter, date)
        case FullScan => scan(expPlan, ed, pool, parameter, date)
      }
  ...
  private def scan(expPlan: LeafExpPlan, ed: EntityData,  pool: CompactEntityDataPool, parameter: Any, date: Date): Set[Entity] = {
    val entities = 
      for(et <- ed.primaryIndex.values;
		e <- et.get(date, ed.metaData, pool).map(_._1) if (expPlan.operator.predicate(e, parameter))
	  ) yield e
	  entities.toSet
  }
  ...
  private def useIndex(expPlan: LeafExpPlan, ed: EntityData, pool: CompactEntityDataPool, parameter: Any, date: Date): Set[Entity] = {
    ...
    val primaryKeys = expPlan.operator.indexCode(index, parameter, date)
    primaryKeys.foldLeft(Set[Entity]()) {(set, pk) =>
      set + ed(pk, date, pool).get 
    }
  }
  
  private def usePrimaryIndex(operator: Operator, ed: EntityData, pool: CompactEntityDataPool, parameter: Any, date: Date): Set[Entity] = {
    operator match {
      case op: EqualsOperator => {
        ed.primaryIndex(parameter).get(date, ed.metaData, pool).map(_._1) match {
